...
3. `tool answer branch` 在动作合法且执行成功时，基于编辑图生成最终答案
...
| `tool answer branch` | 编辑图 + 原始问题 | final answer | 评估工具是否真实带来收益 |
...
字段说明统一放在 [`config/README.md`](../config/README.md)。RL 主链实际依赖 `rl_raw_dir`、`rl_parquet_dir`、`sft_merged_dir` 和 `rl_checkpoint_dir`。
...
```bash
CHARTQA_RL_RAW_DIR=/abs/path/to/raw_data bash data/rl/preprocess_data.sh
```

## 训练流程

当前默认 RL 路径已经切到 `RL-only LoRA`：

1. 从 `sft_merged_dir` 读取 merged SFT 作为基座
2. 训练阶段只更新 RL adapter，不做全量参数更新
3. checkpoint 以 `adapter-first` 形式保存到 `global_step_*/actor`
4. 评测前先显式执行 `RL/merge_rl_lora.py`
...
RL_LORA_R=8 \
RL_LORA_ALPHA=16 \
RL_LORA_DROPOUT=0.05 \
RL_LORA_TARGET_MODULES=q_proj,k_proj,v_proj,o_proj \
bash RL/train.sh
```
...
- 在 merged SFT 基座上挂 RL-only LoRA adapter，并且默认只训练 attention 投影层
...
Base / SFT / RL 三阶段比较时，可以保持同一评测脚本不变，但 RL checkpoint 需要先 merge 成完整模型，再替换 `--model_path`。

## RL LoRA 导出

RL 训练输出的是 adapter-first checkpoint，不能直接喂给 `RL/evaluate_structured.py`。评测前先执行：
...
如果 `adapter_path` 指向的是 `global_step_*/actor`，脚本会自动读取其中的 `metadata.json` 找到基座模型路径。也可以显式传入：

```bash
python RL/merge_rl_lora.py \
  --base_model_path /abs/path/to/sft_merged_model \
  --adapter_path /abs/path/to/rl_checkpoints/global_step_200/actor/adapter \
  --output_path /abs/path/to/rl_merged_model
```
...
| 产物 | 默认位置 | 说明 |
| --- | --- | --- |
| RL checkpoint | `rl_checkpoint_dir/global_step_*/actor` | RL LoRA adapter-first actor checkpoint，包含 `adapter/`、`metadata.json` 和按 rank 保存的 optimizer / RNG 状态 |
| critic checkpoint | `rl_checkpoint_dir/global_step_*/critic` | 仅在启用 critic 的路径下保存 |
