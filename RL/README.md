...
| `rl_parquet_dir` | 预处理后的 `train_full.parquet` / `val_full.parquet` |
...
默认情况下，如果 `trainer.replay.buffer_dir` 没有显式设置，buffer 会保存在：

```text
{save_checkpoint_path}/replay/buffer.jsonl
```
...
| replay buffer | `rl_checkpoint_dir/replay/buffer.jsonl` | 高质量 rollout 样本池 |
