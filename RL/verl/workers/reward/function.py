...
class LLMBatchFunctionRewardManager(FunctionRewardManager):
    ...
    def compute_reward(self, data: DataProto) -> Tuple[torch.Tensor, Dict[str, List[float]]]:
        ...
        for i in range(len(data)):
            ...
            ground_truth.append(data.non_tensor_batch["ground_truth"][i])
            queries.append(data.non_tensor_batch["query"][i])
            penalties.append(data.non_tensor_batch["penalty"][i])

        scores = self.reward_fn(response_str, ground_truth, queries, penalties)
        reward_tensor = torch.zeros_like(data.batch["responses"], dtype=torch.float32)
        reward_metrics = defaultdict(list)
        for i, score in enumerate(scores):
            reward_tensor[i, response_length[i] - 1] = score["overall"]
...
class LLMDoubleBatchFunctionRewardManager(FunctionRewardManager):
    ...
    def compute_reward(self, data: DataProto) -> Tuple[torch.Tensor, Dict[str, List[float]]]:
        ...
        for i in range(len(data)):
            ...
            queries.append(data.non_tensor_batch["query"][i])
            penalties.append(data.non_tensor_batch["penalty"][i])
            rollout_rounds.append(data.non_tensor_batch["rollout_round"][i])
            ids.append(data.non_tensor_batch["figure_id"][i])

        scores = self.reward_fn(response_str, ground_truth, queries, penalties, rollout_rounds, ids)
        reward_tensor = torch.zeros_like(data.batch["responses"], dtype=torch.float32)
        reward_metrics = defaultdict(list)
        for i, score in enumerate(scores):
            reward_tensor[i, response_length[i] - 1] = score["overall"]
